...
#define _MCP23017_INTERRUPT_SUPPORT_  ///< Enables support for MCP23017
                                      ///< interrupts.
...
enum class MCP23017Register : uint8_t {
  ...
  IPOL_A = 0x02,   ///< Configures the polarity on the corresponding GPIO_ port
                   ///< bits for port A.
  IPOL_B = 0x03,   ///< Configures the polarity on the corresponding GPIO_ port
                   ///< bits for port B.
  GPINTEN_A =
      0x04,  ///< Controls the interrupt-on-change for each pin of port A.
  GPINTEN_B =
      0x05,  ///< Controls the interrupt-on-change for each pin of port B.
  DEFVAL_A = 0x06,  ///< Controls the default comparaison value for
                    ///< interrupt-on-change for port A.
  DEFVAL_B = 0x07,  ///< Controls the default comparaison value for
                    ///< interrupt-on-change for port B.
  INTCON_A = 0x08,  ///< Controls how the associated pin value is compared for
                    ///< the interrupt-on-change for port A.
  INTCON_B = 0x09,  ///< Controls how the associated pin value is compared for
                    ///< the interrupt-on-change for port B.
...
inline MCP23017Register operator+(MCP23017Register a, MCP23017Port b) {
  return static_cast<MCP23017Register>(static_cast<uint8_t>(a) +
...
class MCP23017 {
 ...
 public:
  /**
   * Instantiates empty object e.g for static allocation
   */
  MCP23017();
  /**
  ...
  void begin(uint8_t address, TwoWire& bus);
  /**
   * Initializes the chip with the default configuration.
   * Enables Byte mode (IOCON.BANK = 0 and IOCON.SEQOP = 1).
   * Enables pull-up resistors for all pins. This will only be effective
   * for input pins.
   *
   * See "3.2.1 Byte mode and Sequential mode".
   */
   ...
   */
  void portMode(MCP23017Port port, uint8_t directions, uint8_t pullups = 0xFF,
   ...
   * of the MCP23017 definition where a pin is an input if its IODIR bit is set
   * to 1. This library pinMode function behaves like Arduino's standard pinMode
   * for consistency. [ OUTPUT | INPUT | INPUT_PULLUP ]
   */
   ...
   * If intMode is Separated, interrupt conditions on a port will cause its
   * respective INT pin to active. If intMode is Or, interrupt pins are OR'ed so
   * an interrupt on one of the port will cause both pints to active.
   *
