...
void MCP23017::init() {
  ...
  // SEQOP = 	1 : sequential operation disabled, address pointer does not
  // increment DISSLW = 	0 : slew rate enabled HAEN = 	0 : hardware
  // address pin is always enabled on 23017 ODR = 	0 : active driver output
...
void MCP23017::begin(uint8_t address, TwoWire& bus) {
  _deviceAddr = address;
  _bus = &bus;
  begin();
}

void MCP23017::portMode(MCP23017Port port, uint8_t directions, uint8_t pullups,
                        uint8_t inverted) {
...
void MCP23017::writeRegister(MCP23017Register reg, uint8_t portA,
                             uint8_t portB) {
  _bus->beginTransmission(_deviceAddr);
  _bus->write(static_cast<uint8_t>(reg));
  _bus->write(portA);
...
void MCP23017::readRegister(MCP23017Register reg, uint8_t& portA,
                            uint8_t& portB) {
  _bus->beginTransmission(_deviceAddr);
  _bus->write(static_cast<uint8_t>(reg));
  _bus->endTransmission();
